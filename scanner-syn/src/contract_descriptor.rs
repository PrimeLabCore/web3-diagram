...
pub trait ContractDescriptor {
    ...
    fn get_tokens_from_file_path(&self, file_path: &Path) -> (Vec<FunctionInfo>, Vec<TokenStream>);
    fn get_tokens_from_source(&self, src: String) -> (Vec<FunctionInfo>, Vec<TokenStream>);
}
...
impl DefaultContractDescriptor {
    ...
    fn metadata(
    ...
    ) -> syn::Result<(Vec<FunctionInfo>, Vec<TokenStream>)> {
        if let Ok(input) = syn::parse2::<syn::File>(item) {
            ...
            visitor
                ...
                .map(|s| (s, visitor.get_connections()))
    ...
    fn get_contract_info_for_crate(&self) -> ContractInfo {
        ...
        for entry in WalkDir::new(".").into_iter().filter_map(|e| e.ok()) {
            if entry.path().extension().map(|s| s == "rs").unwrap_or(false) {
                println!("\n{}", entry.path().display());
                let (file_functions, connections) = self.get_tokens_from_file_path(entry.path());
                println!("\n{:?}", file_functions);
